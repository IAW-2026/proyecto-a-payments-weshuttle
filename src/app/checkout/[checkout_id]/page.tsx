    ...
    >
      ...
      {!isAdmin && (
        <SectionCard className="print:hidden">
          {data.checkout.status === "CREATED" || data.checkout.status === "PENDING" ? (
            <>
            ...
            <>
              <h2 className="text-lg font-bold text-primary">Información de la transacción</h2>
              <p className="mt-1 text-xs text-slate-gray">
                Detalle del estado del pago y opciones de retorno.
              </p>
            </>
          )}
            ...
            <div className="mt-4 space-y-6">
              {canPayWithMercadoPago ? (
                ...
                <div className="space-y-4">
                  <AlertBanner tone="success" title="¡Pago procesado con éxito!">
                    Tu pago ha sido registrado correctamente. La reserva para tu viaje está confirmada y el comprobante electrónico ya está disponible.
                  </AlertBanner>
                  <div className="flex flex-col gap-3 print:hidden">
                    {riderAppBaseUrl ? (
                      <a
                        href={successReturnUrl}
                        className="inline-flex items-center justify-center rounded-lg bg-primary px-5 py-3 text-center text-sm font-bold text-white shadow-md hover:bg-primary-hover transition duration-200 cursor-pointer"
                      >
                        Volver a Rider App
                      </a>
                    ) : null}
