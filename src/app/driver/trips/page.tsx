...
export default async function DriverTripsPage({ searchParams }: PageProps) {
  ...
  // Fetch only the fast executive summary aggregates for the metrics card
  const summaryData = await getDriverSummaryData(authContext.clerkUserId);
        ...
        <div className="mt-2">
          {activeTab === "viajes" ? (
            <Suspense fallback={<ListSkeleton count={4} />}>
              <DriverTripsTabSection clerkUserId={authContext.clerkUserId} />
            </Suspense>
          ) : (
            <Suspense fallback={<ListSkeleton count={4} />}>
              <DriverSettlementsTabSection clerkUserId={authContext.clerkUserId} q={q} page={page} />
            </Suspense>
          )}
        </div>
      </div>
...
/**
 * Shimmer loader skeleton for tabs lists
 */
...
async function DriverTripsTabSection({ clerkUserId }: { clerkUserId: string }) {
  const trips = await getDriverTripsData(clerkUserId);
...
async function DriverSettlementsTabSection({ clerkUserId, q, page }: { clerkUserId: string; q: string; page: number }) {
  const settlementsData = await getDriverSettlementsData(clerkUserId, q, page);
