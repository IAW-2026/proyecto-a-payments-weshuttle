    ...
    <AppShell role="rider" clerkUserId={authContext.clerkUserId} title="Buscar Pago de Viaje" description="Consulta el estado de cobro y saldo aplicado a tu viaje ingresando el código.">
      <div className="flex flex-col gap-6 max-w-3xl mx-auto">
        ...
        <SectionCard>
          ...
          <Suspense fallback={<ReservationResultsSkeleton />}>
            <ReservationResultsSection clerkUserId={authContext.clerkUserId} reservationId={reservationId} />
          </Suspense>
        </SectionCard>
      </div>
    </AppShell>
...
function ReservationResultsSkeleton() {
  return (
    <div className="mt-6 space-y-6 animate-pulse">
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-5 h-44">
...
async function ReservationResultsSection({ clerkUserId, reservationId }: { clerkUserId: string; reservationId: string }) {
  if (!reservationId) {
    return (
      <div className="mt-6">
  ...
  const data = await getRiderPageData(clerkUserId, reservationId);

  if (!data.latestCheckout && !data.latestCharge) {
    return (
      <div className="mt-6">
